...
class ResourceProfile(models.Model):
    ...
    def get_resources(self, lab=None, working=True):
        """
        ...
        for cls in ResourceManager.RESOURCE_TYPES:
            resources += list(cls.objects.filter(query))
...
class ResourceBundle(models.Model):
    ...
    def get_resources(self):
        my_resources = []
        for cls in ResourceManager.RESOURCE_TYPES:
            my_resources += list(cls.objects.filter(bundle=self))
        return my_resources
...
class Image(models.Model):
    ...
    def in_use(self):
        for cls in ResourceManager.RESOURCE_TYPES:
            for resource in cls.objects.filter(config__image=self):
...
class HostOPNFVConfig(models.Model):
    ...
    host_config = models.ForeignKey(ResourceConfiguration, related_name="host_opnfv_config", on_delete=models.CASCADE)
