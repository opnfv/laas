...
def create_from_API(body, user):
    ...
    data['users'] = [UserProfile.objects.get(pk=user['id']).user
                     for user in booking_info['collaborators']]
...
def _create_booking(data):
    ...
    data['resource_template'] = update_template(data['resource_template'], data['image'], data['hostname'], data['owner'])
