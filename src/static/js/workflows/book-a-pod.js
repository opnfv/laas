...
const steps = {
  ...
  class BookingWorkflow extends Workflow {
    constructor(savedBookingBlob) {
        ...
        this.userTemplates = LibLaaSAPI.getTemplatesForUser() // List<TemplateBlob>

        this.startWorkflow();
    }

    startWorkflow() {
        // this.setBookingDetailEventListeners();
    ...
    onclickConfirm() {
        ...
        if (confirm("Are you sure you would like to create this booking?")) {
            const response = LibLaaSAPI.makeBooking(this.bookingBlob, this.bookingMetaData);
