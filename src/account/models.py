...
class UserProfile(models.Model):
    """
    Simple model used to add some info to the Django User model.
    """
...
class VlanManager(models.Model):
    ...
    def get_vlan(self, count=1):
        """
        returns the ID of available vlans, but does not reserve them.
    ...
    def get_public_vlan(self):
        """
        Returns a reference to an available public network without reserving it
        """
...
class PublicNetwork(models.Model):
    """
    Model for a L2/L3 network that can be used by
    a LaaS user to reach the internet.
    """
