...
def import_host(filenames):
    ...
    NOTE: CONVERT INSPECTION FILES USING convert_inspect_results(["file", "file"]) (original file names not including "-import.yaml" i.e. hpe44) AND FILL IN <NEEDED FIELDS> BEFORE THIS
    ...
    for filename in filenames:
        ...
        if (data["new_profile"]):
            ...
            make_default_template(ResourceProfile.objects.get(name=data["name"]), Image.objects.get(lab_id=data["image"]).id, None, None, False, False, data["owner"], "unh_iol", True, False, data["temp_desc"])
            print(" Template: " + data["temp_name"] + " created!")
        add_server(ResourceProfile.objects.get(name=data["name"]), data["hostname"], data["interfaces"], data["lab"], data["vendor"], data["model"])
